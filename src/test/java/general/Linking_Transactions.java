...
//public class Linking_Transactions extends Base {
	...
	private static void Link_Invoice() throws Exception {
		...
		try {
			...
			wait.until(ExpectedConditions.visibilityOfElementLocated(
					By.xpath("(//*[@resource-id='com.impossible_research.sandbox.starboard:id/transaction']) [3]")))
					.click();
	...
	private static void Link_Expense_Categories() throws Exception {
...

		{
			...
			wait.until(ExpectedConditions.visibilityOfElementLocated(
					By.xpath("(//*[@resource-id='com.impossible_research.sandbox.starboard:id/head']) [5]"))).click();

			// Select & Print 1st Expense Category to link
			wait.until(ExpectedConditions.visibilityOfElementLocated(
					By.xpath("(//*[@resource-id='com.impossible_research.sandbox.starboard:id/category_name']) [1]")))
					.click();
	...
	private static void Notes_Add() throws Exception {
...

		{
			...
			// Add notes Title
			wait.until(
					ExpectedConditions.visibilityOfElementLocated(By.xpath("//android.widget.EditText[@text='Title']")))
					.click();

			driver.findElement(By.xpath("//android.widget.EditText[@text='Title']")).sendKeys("Note" + " " + timestamp);
